...
def extract_categories_urls(): # Récupère la liste des catégories (nom + url) pour les stocker dans [categories_titles] et [categories_url]
...
def csv_create(datas_path, category_url): # Créer un fichier .csv par catégorie, crée une en-tête et y ajoute les données de toutes les pages
...
def scraping (url_to_work): # Ouvre l'URL de la catégorie puis ouvre chaque page d'article, récupère toutes les données pour les stocker dans [scraped_books]
...
def browse_urls(categories_urls): # Parcourt toutes les categories, teste si il y a plusieurs pages, renvoie le contenu
